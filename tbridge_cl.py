"""
A command-line wrapper for TBRIDGE with full arguments included.
"""
...
provided_bgs = None if args.BGS is None else tbridge.cutouts_from_file(args.BGS)

if str(args.LINEAR).lower() in ('true', 't', '1'):
    linear = True
elif str(args.LINEAR).lower() in ('false', 'f', '0'):
    linear = False
else:
    linear = True
...
tbridge.pipeline(config, separate_mags=None, max_bins=1, provided_bgs=provided_bgs, linear=linear)
